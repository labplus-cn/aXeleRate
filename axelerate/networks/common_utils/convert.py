...
if SYSTEM=='Linux':
    k210_converter_path=os.path.join(os.path.dirname(__file__),"ncc_linux","ncc")
    k210_converter_download_path=os.path.join(os.path.dirname(__file__),'ncc-linux-x86_64.tar.xz')
    nncase_download_url="https://github.com/kendryte/nncase/releases/download/v0.1.0-rc5/ncc-linux-x86_64.tar.xz"

elif SYSTEM =='Windows':
    k210_converter_path=os.path.join(os.path.dirname(__file__),"ncc_win","ncc.exe")
    k210_converter_download_path=os.path.join(os.path.dirname(__file__),'ncc-win7-x86_64.zip')
    nncase_download_url="https://github.com/kendryte/nncase/releases/download/v0.1.0-rc5/ncc-win7-x86_64.zip"
...
class Converter(object):
        ...
        if 'k210' in converter_type:
            if os.path.exists(k210_converter_path):
                ...
                if SYSTEM =='Linux':
                    ...
                    tar_file.extractall(os.path.join(os.path.dirname(__file__),"ncc_linux"))
                    tar_file.close()
                elif SYSTEM =='Windows':
                    import zipfile
                    zip_file = zipfile.ZipFile(k210_converter_download_path)
                    zip_file.extractall(os.path.join(os.path.dirname(__file__),"ncc_win"))
                    zip_file.close()
    ...
    def convert_k210(self,model_path,dataset_path):
        ...
        if result.returncode == 0:
            print("Convert k210 model completed. {}" .format(output_name))
        # print(result.returncode)
